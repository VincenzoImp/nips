...
```json
{
  ...
  "terms_of_service": <a link to a text file describing the relay's terms of service>,
...
Note: this field is self-reported and advisory only. Clients that require specific functionality SHOULD probe the relay directly rather than relying solely on this declaration.
...
The relay owner/admin MAY choose to link to a terms of service document. The value MUST be a URL pointing to a human-readable text file.
...
- The array SHOULD contain no more than 8 entries to keep relay descriptions concise.
- There is no fixed vocabulary; operators choose values that best describe their relay.

The following values are RECOMMENDED for common relay types:

| Attribute | Meaning |
|-----------|---------|
| `Public` | Accepts events from anyone without authentication or payment |
| `Paid` | Requires payment to write or read |
| `Auth` | Requires [NIP-42](42.md) authentication |
| `Search` | Supports [NIP-50](50.md) full-text search |
| `Community` | Fosters a local community (e.g. [NIP-29](29.md) groups) |
| `Ephemeral` | Does not persist events beyond the current session |
| `Nsfw` | Hosts adult or otherwise sensitive content |
| `Indexer` | Aggregates events from multiple relays |
| `Blob` | Serves binary data (e.g. [NIP-95](95.md)) |
| `Inbox` | Accepts writes from many users; intended for receiving DMs or mentions |
| `Outbox` | Optimized for reading by followers; intended for publishing a user's own events |

Relay monitor tools (see [NIP-66](66.md)) SHOULD propagate `attributes` entries as `W` tags in relay discovery events to enable attribute-based discovery.
...
should expect that requests exceeding these *practical* limitations
